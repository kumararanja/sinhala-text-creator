    ...
    try:
        ...
        conn.close()
            ...
            created_date = user['created_at'].strftime('%Y-%m-%d') if user['created_at'] else 'N/A'
                        return f"""### 👤 {user['email']}- **Plan:** {user['plan'].upper()}- **This Month:** {user['monthly_generations']} used | **{remaining} remaining**- **All Time:** {user['total_generations']} generations- **Member Since:** {created_date}
            """
...
def get_admin_stats() -> str:
    ...
    try:
        ...
        today_signups = cursor.fetchone()['today_count']
                conn.close()
                # Format the report
        report = f"""# 📊 **ADMIN DASHBOARD**## 👥 **User Statistics**- **Total Users:** {total_users}- **New Today:** {today_signups}- **Active (30 days):** {active_users}## 💎 **Users by Plan**"""
                for plan in plan_stats:
            report += f"\n- **{plan['plan'].upper()}:** {plan['count']} users"
                report += f"""## 🎨 **Generation Statistics**- **Total All-Time:** {gen_stats['total_gens']} generations- **Used This Month:** {gen_stats['monthly_gens']} generations- **Average per User:** {gen_stats['total_gens'] // max(total_users, 1)} generations## 🆕 **Recent Users (Latest 15)**| Email | Plan | Joined | Monthly | Total ||-------|------|--------|---------|-------|"""
