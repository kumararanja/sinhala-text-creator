...
for name, path in FONT_PATHS.items():
    try:
        ...
        # Try without fonts/ prefix if running locally
        try:
            path_without_prefix = path.replace("fonts/", "")
            ImageFont.truetype(path_without_prefix, 20)
            fonts_available[name] = path_without_prefix
        except:
            pass
    ...
    with gr.Blocks(title="Sinhala Text Creator", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Group(visible=False) as main_app:
            ...
            with gr.Tabs():
                ...
                # --- NEW DOWNLOAD SECTION ---  <--- THIS BLOCK MUST START AT THE SAME LEVEL
